...
function rng_seed_int(a) {
    ...
    rng_pool[rng_pptr++] ^= 255 & a >> 8,
    rng_pool[rng_pptr++] ^= 255 & a >> 16,
    rng_pool[rng_pptr++] ^= 255 & a >> 24,
...
function(a, b) {
    ...
    d = 0;
    b.fn.PasswordWidget = function(e) {
        $.extend(c, e || {});
        var f, g, h = $(this),
        ...
        p = h.attr("v-password-widget"),
        pp=p.indexOf('?')>=0?'&width=':'?width=',
        q = document.body.clientWidth,
        r = [];
        q > 570 && (q = 570),
        a.orientation && 0 != a.orientation && (q = .6 * document.body.clientWidth),
        i ? l.setPublic(i, k) : alert("please set modulus_hex!"),
        h.css({
            color: "#999999",
        ...
        h.html("正在加载..."),
        $("#_PassWordWidget" + c.KeyType).length ? f = $("#_PassWordWidget" + c.KeyType) : (f = $('<div id="_PassWordWidget' + c.KeyType + '" onselectstart="return false;" style="display:none;position:fixed;left:0px;bottom: 0px;z-index: 9003;width: 100%; height:' + 333 * q / 576 + 'px;background-color:#D9DADF;-moz-user-select: none;-webkit-user-select: none;-ms-user-select: none;-khtml-user-select: none; user-select: none;"></div>'), $("body").append(f).append('<div id="_PassWordShadow" style="display: none;position: absolute;top: 0;left: 0;width: 100%;height: 100%;overflow: hidden;z-index: 8999;opacity: 0;"></div>'));
        var s = function() {
            $("#_PassWordWidget" + c.KeyType + ",_PassWordShadow").remove(),
            h.html(m),
            h.val(m)
        },
        t = function(a) {
            $("#_PassWordShadow").hide(),
            f.hide().find("#LowercaseDiv,#UppercaseDiv,#NumberDiv").hide(),
            a.attr("data-ciphertext", l.encrypt(r.join(","))),
            a.attr("data-passwordlength", r.length),
            0 === r.length && (a.css("color", "#999999"), a.html(m),a.val(m)),
            a.removeClass('focus'),
        ...
        u = function(a) {
        	...
        	var $widget= $("#_PassWordWidget" + c.KeyType);
        	if($widget.is(':visible')){
        		a.attr('readonly',true).addClass('focus');
            	$(document).trigger('keyboard.show',{width:$widget.width(),height:$widget.height()});
        	}
        	
            $("#_PassWordShadow").unbind("click").bind("click",
            function() {
                t(a)
            }),
            f.find("#LowercaseDiv>div,#UppercaseDiv>div,#NumberDiv>div").bind("touchstart",
            function() {
                $(this).css(o)
            }).unbind("touchend").bind("touchend",
            function() {
                var b = $(this);
                if (b.siblings("div").css({
                    ...
                    if (a.is('input')) {
                        r.length < n && (r.push(b.attr("data-keyIndex")), 1 === r.length ? a.val("*") : a.val(a.val() + "*"));
                    } else {
                    ...
                    case "F2":
                    	 if (a.is('input')) {
                    		 r.length > 0 && a.val(a.val().substr(1)) && r.pop(),
                             0 == r.length && a.val(m);
                    	 }else{
                    		 r.length > 0 && a.text(a.text().substr(1)) && r.pop(),
                             0 == r.length && a.text(m);
                    	 }
                        break;
                    case "F3":
                        f.find("#" + g).show(),
                        f.find("#NumberDiv").hide();
                        break;
                    case "F4":
                        f.find("#LowercaseDiv,#UppercaseDiv").hide(),
                        f.find("#NumberDiv").show(),
                        g = b.parent().attr("id");
                        break;
                    case "F5":
                        t(a)
        ...
        d || c.UseCache && f.children().length ? h.val(m) : (d++, b.post(p + pp + q).success(function(a) {
            if (d--, f.html('<div style="position:relative;top:0px;left:0px;width: 100%;height: 100%;z-index: 9003;">' + a + "</div>"), f.css("height", f.children().children("div:first").height()), v(), h.html("请输入密码"), "true" == j) {
                if (!f.children().length) return;
                h.css("color", "#000000"),
                $("#_PassWordShadow").show(),
                f.show().children().children("div:first").show(),
                u(h)
            }
        }).error(function() {
            d--,
...
function(a, b) {
    ...
    if (b) {
        ...
        c.directive("vPasswordWidget", ["$log",
        function() {
            return {
                restrict: "AE",
                link: function(a, c, d) {
                    var e = $.extend({},
                    b.fromJson(d.options || {}));
